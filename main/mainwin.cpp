...
Mainwin::Mainwin() {
    ...
    createmenu->append(*menuitem_server);
      ...
      //        M A N A G E R
    // (Owner, Manager) “Add a New Employee” Append Manager to the Create menu
    ...
    menuitem_order->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_vorder_click));
    viewmenu->append(*menuitem_order);

    //           C O N T A I N E R
    // Append Order to the View menu
    Gtk::MenuItem *menuitem_cont = Gtk::manage(new Gtk::MenuItem("Containers", true));
    menuitem_cont->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_vcont_click));
    viewmenu->append(*menuitem_cont);

    //           S C O O P
    // Append Order to the View menu
    Gtk::MenuItem *menuitem_scoop = Gtk::manage(new Gtk::MenuItem("Scoops", true));
    menuitem_scoop->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_vscoop_click));
    viewmenu->append(*menuitem_scoop);

    //           T O O P I N G
    // Append Order to the View menu
    Gtk::MenuItem *menuitem_top = Gtk::manage(new Gtk::MenuItem("Toppings", true));
    menuitem_top->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_vtop_click));
    viewmenu->append(*menuitem_top);

    //           P R O F I T/L O S S
    // Append Order to the View menu
    Gtk::MenuItem *menuitem_pl = Gtk::manage(new Gtk::MenuItem("Profit/Loss", true));
    menuitem_pl->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_pl_click));
    viewmenu->append(*menuitem_pl);
...
void Mainwin::on_pl_click(){

    std::string msg="";
    if(_emp->cash_register()<0)
    {
        std::string msg="We are making a loss of: ";
    }

    if(_emp->cash_register()>=0)
    {
        std::string msg="We are making a profit of: ";
    }
    Gtk::MessageDialog *dialog = new Gtk::MessageDialog("Profit/Loss");
    dialog->set_secondary_text(msg+std::to_string(_emp->cash_register()), true);
    dialog->run();
...
void Mainwin::on_vorder_click(){

int ord = select_order(Mice::Order_state::Unfilled);
auto order=_emp->order(ord);
   ...
   /*     
    Gtk::MessageDialog *dialog = new Gtk::MessageDialog("Order Info");
...
void Mainwin::on_vcont_click(){

    Gtk::Dialog *dialog = new Gtk::Dialog();
    dialog->set_title("Containers");
    ...
    Gtk::Image *image = new Gtk::Image{"cont.png"};
    image->show();
...
void Mainwin::on_vscoop_click(){
  Gtk::Dialog *dialog = new Gtk::Dialog();
    dialog->set_title("Scoops");
    ...
    Gtk::Image *image = new Gtk::Image{"scoop.png"};
    image->show();
    dialog->get_vbox()->pack_start(*image);
...
void Mainwin::on_vtop_click(){
  Gtk::Dialog *dialog = new Gtk::Dialog();
    ...
    Gtk::Image *image = new Gtk::Image{"top.png"};
    image->show();
    dialog->get_vbox()->pack_start(*image);
